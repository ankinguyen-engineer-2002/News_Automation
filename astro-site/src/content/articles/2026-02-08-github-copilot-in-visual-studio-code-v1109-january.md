---
title: "GitHub Copilot in Visual Studio Code v1.109 – January Release"
title_vi: ""
source: "GitHub Changelog"
url: "https://github.blog/changelog/2026-02-04-github-copilot-in-visual-studio-code-v1-109-january-release"
topic: "github"
date: "2026-02-08"
excerpt: "This January 2026 release brings significant improvements to GitHub Copilot in Visual Studio Code with agent-driven workflows, improvements to agent session management, and the introduction of agent..."
excerpt_vi: ""
number: 66
publishDate: "2026-02-08T00:00:00Z"
---

This January 2026 release brings significant improvements to GitHub Copilot in Visual Studio Code with agent-driven workflows, improvements to agent session management, and the introduction of agent support for Claude… The post GitHub Copilot in Visual Studio Code v1.109 – January Release appeared first on The GitHub Blog .
